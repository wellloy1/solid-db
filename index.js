...
const [_db, _updateDb] = createStore({
...
export class Table {
    ...
    #updateSignal(signal) {
        ...
        const updated = !!_db[signal][0];
        actions[updated]();
    }

    #createTable(name, schema, data) {
        _updateDb(name, { name, data, indexes: {} });
    }
    ...
    onChange(fn) {
        createEffect(() => {
            if (_db.__signalChange[0] === 1) return;
            fn();
        });
    }
    ...
    onUpdate(fn) {
        createEffect(() => {
            if (_db.__signalUpdate[0] === 1) return;
            fn();
        });
    }
    ...
    onRemove(fn) {
        createEffect(() => {
            if (_db.__signalRemove[0] === 1) return;
            fn();
        });
    }
    ...
    onInsert(fn) {
        createEffect(() => {
            if (_db.__signalInsert[0] === null) return;
            fn();
        });
    }

    list() {
        _db.__signalUpdate[0];
